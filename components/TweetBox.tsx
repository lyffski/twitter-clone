...
function TweetBox({ setTweets }: Props) {
  ...
  return (
    <div className='flex space-x-2 p-5'>
        <img className='h-14 w-14 rounded-full object-cover mt-4' src={session?.user?.image || 'https://links.papareact.com/gll'}/>

        <div className='flex flex-1 items-center pl-2'>
            <form className='flex flex-1 flex-col'>
                <input value={input} onChange={(e) => setInput(e.target.value)} type="text" className="h-24 w-full text-xl outline-none placeholder:text-xl" placeholder='Whats Happening?'/>
                <div className='flex items-center'>
                    <div className="flex space-x-2 text-twitter flex-1">
                        ...
                        <CalendarIcon className='h-5 w-5 cursor-pointer transition-transform duration-150 ease-out hover:scale-150' />+<LocationMarkerIcon className='h-5 w-5'/>
                    </div>
                ...
                {imageUrlBoxIsOpen && (
                    <form className='mt-5 flex rounded-lg bg-twitter/80 py-2 px-4'>
                        ...
                        <button onClick={addImageToTweet} type="submit" className='font-bodl text-white'>Add Image</button>
                    </form>
                )}
